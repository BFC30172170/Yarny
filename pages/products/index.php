...
// Get static page assets server-side
$tags = Tag::getTags($con);
$cats = Category::getCategories($con);

// server-side static render functions for creating category tree with padding
...
{
    // When category selection is changed, re-render client side products with getProducts func
...
<!-- Client side fetch products and render these with alpine.js -->
<div x-data="{
    products: null,
    category: '',
    tags: [],
    ...
    <div class="grid grid-cols-12">
            ...
            foreach ($tags as $tag) {
                ...
                <button class="flex items-center text-sm py-1" :class="tags.includes(<?= $tag->id ?>) ? 'font-black text-teal-500' : ''"
                    ...
                    <div class="self-center rounded-lg w-4 h-4 mr-2 "
                        :class="tags.includes(<?= $tag->id ?>) ? 'bg-teal-500' : 'border-2 b-teal-500'"></div>
                    <?= $tag->name ?>
                </button>
                <?php
            }
        ...
        <!-- Render grid view with products -->
        ...
        <!-- Render list view with products -->
        <div x-show="products.length > 0 && list"
